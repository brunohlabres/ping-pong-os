...
void moverBody(void *arg)
{
  ...
  printf("T%02d will move %2d blocks between block %3d and block %3d\n",
  ...
  for (i = 0; i < blocksPerTask; i++)
  {
    ...
    printf("T%02d will read block %3d\n", task_id(), block_orig);
    if (disk_block_read(block_orig, buffer1) == 0)
      printf("T%02d read  block %3d\n", task_id(), block_orig);
    else
      printf("T%02d error reading block %3d\n", task_id(), block_orig);
    ...
    printf("T%02d block %3d has: [", task_id(), block_orig);
    for (j = 0; j < blockSize; j++)
      printf("%c", buffer1[j]);
    printf("]\n");

    // mostra o conteudo do bloco b2
    printf("T%02d block %3d has: [", task_id(), block_dest);
    for (j = 0; j < blockSize; j++)
      printf("%c", buffer2[j]);
    printf("]\n");

    // escreve o bloco b1 no disco
    printf("T%02d will write block %3d\n", task_id(), block_dest);
